...
export const buildFormData = ({ form, courses, exams, companies, reviews, imageFiles }) => {
  ...
  fd.append("exams", JSON.stringify(exams || []));
...
export default function CollegeForm({
  ...
  const [exams, setExams] = useState(mode === "edit" && initialData ? initialData.exams || [] : []);
  ...
  const handleSubmit = () => {
    if (!validate()) return;
    const fd = buildFormData({ form, courses, exams, companies, reviews, imageFiles });
    onSubmit?.(fd);
  };
  ...
  const handleReset = () => {
    setForm({ ...EMPTY_FORM });
    setCourses([]); setExams([]); setCompanies([]); setReviews([]);
    clearImages();
  };
  ...
  return (
    <div className="flex flex-col gap-0">
      ...
      <FormCard title="Entrance Exams Accepted" index={4} color="purple">
        <div className="flex flex-col gap-3">
          {exams.length === 0 && (
            <div className="text-center py-8 text-slate-400 text-[13px]">
              <FileText size={32} className="mx-auto mb-2 opacity-30" />No exams yet. Add one below.
            </div>
          )}
          {exams.map((exam, i) => (
            <div key={i} className="flex gap-2 items-center bg-white border border-slate-200 rounded-2xl p-3 shadow-sm">
              <div className="w-6 h-6 rounded-lg bg-violet-500 text-white flex items-center justify-center text-[10px] font-black shrink-0">{i + 1}</div>
              <div className="flex-1">
                <Input
                  placeholder="e.g. JEE Main, CAT, NEET, etc."
                  value={exam}
                  onChange={(e) => {
                    const val = e.target.value;
                    setExams(p => p.map((x, xi) => xi === i ? val : x));
                  }}
                />
              </div>
              <button
                type="button"
                onClick={() => setExams(p => p.filter((_, xi) => xi !== i))}
                className="w-9 h-9 rounded-xl flex items-center justify-center text-rose-400 hover:bg-rose-50 hover:text-rose-600 transition-all shrink-0 border border-transparent hover:border-rose-100"
              >
                <Trash2 size={13} />
              </button>
            </div>
          ...
          <button
            type="button"
            onClick={() => setExams(p => [...p, ""])}
            className="flex items-center justify-center gap-2 w-full py-3 rounded-2xl border-2 border-dashed border-slate-200 text-slate-400 text-[13px] font-bold hover:border-violet-500 hover:text-violet-500 hover:bg-violet-50/5 transition-all"
          >
      ...
      {/* ⑤ College Ratings */}
      <FormCard title="College Ratings" index={5}>
      ...
      {/* ⑥ Reviews */}
      <FormCard title="Reviews" index={6}>
